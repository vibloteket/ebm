...
function coordinateMap(reference){return `<div class="coordinate-map" aria-label="200 by 200 local tile coordinates">${reference.ports.map(port=>{const [x,y]=port.point;return `<span class="port ${port.kind}" style="left:${x/2}px;top:${y/2}px">${escapeHtml(port.name)}</span>`}).join("")}<span class="axis" style="left:5px;top:5px">(0, 0)</span><span class="axis" style="right:5px;bottom:5px">(${reference.tileSize}, ${reference.tileSize})</span></div><p>Coordinates are local to each ${reference.tileSize} × ${reference.tileSize} tile. Green ports are inputs; red ports are outputs. Physical build points may extend at most 10 units beyond the edge.</p>`}
