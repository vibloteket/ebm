...
class MyTile(TileBase):
  ...
  const sections=[
    ...
    ["ports","Ports & coordinates",`${coordinateMap(reference)}<p>Coordinates are local to the tile. <code>(0, 0)</code> is the upper-left corner; x increases to the right and y increases downward. Green ports are inputs and red ports are outputs.</p><h4>Port positions and openings</h4><table class="api-table"><thead><tr><th>Port</th><th>Kind</th><th>Center</th><th>Full opening</th><th>Ball-center range</th></tr></thead><tbody>${reference.ports.map(port=>`<tr><td><code>${port.name}</code></td><td>${port.kind}</td><td><code>(${port.point[0]}, ${port.point[1]})</code></td><td><code>${portRange(port,rules.aperture)}</code></td><td><code>${centerRange(port,rules.centerRange)}</code></td></tr>`).join("")}</tbody></table><p>The full opening includes the whole ${rules.aperture}-unit gap. Since a ball has radius ${rules.ballRadius}, its center uses the narrower range shown in the last column.</p><h4>Ball size and arrival</h4>${list([
      ...
      `All tile-owned geometry must fit completely inside 0–${reference.tileSize}, including segment end caps, circle radii and polygon bevels. There is no construction margin, even at ports. Sensors and visual-only segments follow the same rule.`,
      "For an edge rail of radius r, place its centreline r units inside the edge and keep its endpoints r units away from the other edges. Invalid construction raises GeometryBoundsError (a ValueError) immediately.",
      "Body-local coordinates may be negative: attached shapes are checked after applying the body's position and rotation. Position, angle and visual-endpoint setters reject out-of-bounds edits without changing the object.",
    ])}`],
    ["validation","Validation rules",`<p><strong>Validate flow</strong> runs the current strict, concurrent flow test:</p>${list([
      ...
      "Both the single-tile and 3 × 3 tests also check all owned geometry after build, after each update, after every physics step and after delayed resumes. Hidden and paused shapes are included; balls use the separate port rules.",
      "The first geometry violation stops the test and reports the object, tile, simulation time, edge and overflow distance. Only a tiny floating-point tolerance is allowed, not an authoring margin.",
      "Runtime checks cover the simulated test trajectory, not every possible future motion. They do not clamp or move geometry and are not run in the live machine's frame loop.",
