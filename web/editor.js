...
const PY_FILES=["__init__.py","ports.py","random_utils.py","tile_base.py","tile_api.py","tile_catalog.py","editor_console.py","tiles/__init__.py","tiles/builtin/__init__.py","tiles/builtin/powered_channel.py","tiles/builtin/reference_router.py","tiles/contributed/__init__.py","routes.py","validator.py","debug_demo.py","editor_runtime.py","editor_preview.py"];
...
TILE_CLASS = MyTile
`;
const ids=["tile-select","status","validation-results","draft-state","cursor-position","preview","loading","tile-title","tile-id","tile-author","tile-origin","console-output","console-count"];
const els=Object.fromEntries(ids.map(id=>[id.replaceAll("-","_"),document.getElementById(id)]));
let pyodide,manifest,currentTile,originalSource="",mode="single",paused=false,saveTimer,consoleTimer,consoleLines=[],codeEditor;
...
function renderValidation(r){
  ...
  const summary=`<table class="validation-table"><thead><tr><th>Result</th><th>Entered</th><th>Exited</th><th>Active</th><th>Peak</th><th>B0</th><th>L1</th><th>R1</th><th>Invalid/lost</th></tr></thead><tbody><tr><td class="${r.ok?"pass":"fail-text"}">${r.ok?"PASS":"FAIL"}</td><td>${r.balls_spawned}</td><td>${r.exited}</td><td>${r.active}/${r.max_active_allowed}</td><td>${r.peak_active}/${r.max_active_allowed}</td><td>${r.output_counts.B0}</td><td>${r.output_counts.L1}</td><td>${r.output_counts.R1}</td><td>${r.invalid+r.lost}</td></tr></tbody></table>`;
...
async function validate(){
  ...
  try{
    ...
    els.validation_results.querySelectorAll(".replay-failure").forEach(button=>button.addEventListener("click",()=>{const detail=failures.get(button.dataset.failureId);if(detail){mode="single";document.querySelectorAll("[data-mode]").forEach(b=>b.classList.toggle("active",b.dataset.mode==="single"));pyodide.globals.get("refresh_preview")("single");pyodide.globals.get("replay_validation_failure")(JSON.stringify(detail));button.textContent="Replaying…"}}));
