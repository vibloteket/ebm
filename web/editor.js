...
`;
const ids=["tile-select","status","validation-results","draft-state","cursor-position","preview","loading","tile-name","tile-author","tile-origin","console-output","console-count"];
...
function authorFromSource(source){return source.match(/^\s*author\s*=\s*["']([^"']*)["']/m)?.[1]||""}
...
function replaceAuthor(value){const safe=value.replaceAll("\\","\\\\").replaceAll('"','\\"'),source=getSource(),pattern=/^(\s*author\s*=\s*)["'][^"']*["']/m;if(pattern.test(source))setSource(source.replace(pattern,`$1"${safe}"`));saveDraft()}
function updateMetadata(source,origin,displayName){els.tile_name.textContent=displayName||classNameFromSource(source);els.tile_author.value=authorFromSource(source);els.tile_origin.textContent=origin}
async function loadTile(id){saveDraft();if(id===NEW_ID){currentTile={id:NEW_ID,title:"My Tile",author:""};originalSource=NEW_SOURCE}else{currentTile=manifest.tiles.find(tile=>tile.id===id);originalSource=await fetchText(`./${currentTile.source}`)}const draft=localStorage.getItem(draftKey(id));setSource(draft??originalSource);els.draft_state.textContent=draft===null?(id===NEW_ID?"New skeleton":"Original"):"Modified locally";updateMetadata(getSource(),id===NEW_ID?"New local tile":`Based on ${currentTile.title}`,currentTile.title);await runSource()}
async function runSource(){setStatus("Building tile…");els.validation_results.innerHTML="";codeEditor.setErrorLine(null);try{const result=JSON.parse(pyodide.globals.get("compile_source")(getSource()));if(!result.ok){codeEditor.setErrorLine(result.line);setStatus(`${result.type||"Build error"}${result.line?` at line ${result.line}`:""}`,"fail",result.message);return}updateMetadata(getSource(),currentTile.id===NEW_ID?"New local tile":`Based on ${currentTile.title}`,result.displayName);pyodide.globals.get("refresh_preview")(mode);setStatus(`Running ${result.displayName}`,"ok",`by ${result.author}`)}catch(error){setStatus("Runtime error","fail",error.stack||String(error))}finally{drainConsole()}}
...
async function validate(){
  ...
  try{
    ...
    els.validation_results.querySelectorAll(".replay-failure").forEach(button=>button.addEventListener("click",()=>{const detail=failures.get(button.dataset.failureId);if(detail){setView("validation");pyodide.globals.get("refresh_preview")("single");pyodide.globals.get("replay_validation_failure")(JSON.stringify(detail));els.validation_results.querySelectorAll(".replay-failure").forEach(item=>item.textContent="Replay");button.textContent="Replaying…"}}));
...
function sourceChanged(source){els.tile_name.textContent=classNameFromSource(source);els.tile_author.value=authorFromSource(source);clearTimeout(saveTimer);saveTimer=setTimeout(saveDraft,250)}
...
els.tile_author.addEventListener("change",()=>replaceAuthor(els.tile_author.value));
document.querySelectorAll("[data-view]").forEach(button=>button.onclick=()=>setView(button.dataset.view));
document.getElementById("console-clear").onclick=()=>{consoleLines=[];pyodide?.globals.get("clear_editor_console")?.();renderConsole()};document.getElementById("console-toggle").onclick=event=>{const panel=document.getElementById("console-panel"),collapsed=panel.classList.toggle("collapsed");document.querySelector(".code-pane").classList.toggle("console-collapsed",collapsed);event.currentTarget.setAttribute("aria-expanded",String(!collapsed))};document.getElementById("run-button").onclick=runSource;document.getElementById("validate-button").onclick=validate;document.getElementById("pause-button").onclick=event=>{paused=!paused;pyodide.globals.get("set_preview_paused")(paused);event.currentTarget.textContent=paused?"Resume simulation":"Pause simulation";event.currentTarget.setAttribute("aria-pressed",String(paused))};document.querySelectorAll("[data-mode]").forEach(button=>button.onclick=()=>{mode=button.dataset.mode;document.querySelectorAll("[data-mode]").forEach(b=>b.classList.toggle("active",b===button));pyodide.globals.get("refresh_preview")(mode)});els.tile_select.onchange=()=>loadTile(els.tile_select.value);document.getElementById("reset-button").onclick=()=>{if(confirm("Discard the local draft and restore the skeleton/original source?")){setSource(originalSource);saveDraft();updateMetadata(getSource(),currentTile.id===NEW_ID?"New local tile":`Based on ${currentTile.title}`,currentTile.title);runSource()}};document.getElementById("download-button").onclick=()=>{const source=getSource(),blob=new Blob([source],{type:"text/x-python"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=filenameFromClass(classNameFromSource(source));a.click();URL.revokeObjectURL(a.href)};document.getElementById("import-button").onclick=()=>document.getElementById("import-file").click();document.getElementById("import-file").onchange=async e=>{const file=e.target.files[0];if(file){setSource(await file.text());saveDraft();updateMetadata(getSource(),`Imported from ${file.name}`);runSource()}e.target.value=""};
