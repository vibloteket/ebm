...
import {initializeApiReference} from "./api-reference.js?v=0.74";
...
async function writePackage(){await loadPythonPackage(pyodide)}
...
async function runSource(){setStatus("Building tile…");els.validation_results.innerHTML="";codeEditor.setErrorLine(null);try{const result=JSON.parse(pyodide.globals.get("compile_source")(getSource()));if(!result.ok){codeEditor.setErrorLine(result.line);setStatus(`${result.type||"Build error"}${result.line?` at line ${result.line}`:""}`,"fail",result.message);return}pyodide.globals.get("refresh_preview")(mode);setStatus(`Running ${result.displayName}${result.enabled===false?" (disabled in machine)":""}`,"ok",`by ${result.author}`)}catch(error){setStatus("Runtime error","fail",error.stack||String(error))}finally{drainConsole()}}
