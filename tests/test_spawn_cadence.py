...
def test_all_source_defaults_share_one_ball_per_second_tile_budget():
    assert TILE_SPAWN_INTERVAL == SPAWN_INTERVAL == 1.0
    assert INPUT_SPAWN_INTERVAL == BOUNDARY_SPAWN_INTERVAL == 2.0
    assert inspect.signature(validate_tile_flow).parameters['spawn_interval'].default == 1.0
    assert inspect.signature(validate_repeated_flow).parameters['spawn_interval'].default == 2.0


def test_single_validator_spawns_one_per_second_across_both_inputs():
    result = validate_tile_flow(default_tile, balls=8)
    assert result.ok, result.to_dict()
    arrivals = sorted(result.details, key=lambda item: item['id'])
    assert [item['spawned_at'] for item in arrivals] == pytest.approx(list(range(8)), abs=.01)
    assert [item['entry'] for item in arrivals] == ['T0', 'L0'] * 4


def test_repeat_validator_spawns_five_per_boundary_input_in_ten_seconds():
    ...
    assert result.balls_spawned == 6 * 5
